  <script>
    (function () {
      ...
      function spawnParkedCars() {
        const slots = parkingBays.slice();
        for (let i = 0; i < 15; i++) {
          const idx = Math.floor(Math.random() * slots.length);
          const bay = slots.splice(idx, 1)[0];
          const m = buildAiCar(AICOLORS[Math.floor(Math.random() * AICOLORS.length)]);
          m.position.set(bay.x, .38, bay.z);
          m.rotation.y = bay.ang;
          const v = {
            m, chain: AI_CHAINS[0], // fallback chain
            wpi: 1, loopWpi: 1,
            spd: 0,
            stalled: false,
            state: 'parked',
            bay: bay, unparkTimer: 10 + Math.random() * 60
          };
          bay.occupied = v;
          aiVehicles.push(v);
        }
      ...
      function buildAiCar(color) {
        ...
        /*HEAD*//*TAIL*/[[-0.62, .5, 2.05], [0.62, .5, 2.05]].forEach(([x, y, z]) => {
        ...
        /*TAIL*/[[-0.62, .5, 2.05], [0.62, .5, 2.05]].forEach(([x, y, z]) => {
        ...
        aiVehicles.forEach((v, vi) => {
          ...
          if (v.state === 'parked') {
            ...
            if (v.unparkTimer <= 0) {
              v.state = 'leaving';
              v.spd = 0;
              // Choose a destination vertical road (V1 or V2)
              // Choose a destination: V1 or Roundabout connection
          ...
          if (v.state === 'leaving') {
            ...
            v.spd = isParallel ? 2.5 : -2.5; // forward for parallel, reverse for angle
            ...
            if (distToAisle < 0.2) {
              ...
              // Turn while leaving to face the exit
          ...
          if (playerCar) {
            ...
            if (fwd > -2 && fwd < 15 && lat < 2.5) {
              // If player is directly in front, AI should match speed or stop to "follow"
              // Overtaking logic would belong here, but stopping firmly handles the "follow" requirement safely.
              desiredSpd = 0;
            } else if (fwd > 15 && fwd < 30 && lat < 2.5) {
              // Slow down gracefully
              desiredSpd = Math.min(desiredSpd, Math.max(0, ph.speed - 0.5));
            }
          }
          ...
          if (distToTarget < target.acc) {
            v.wpi++;
            if (v.wpi >= chain.length) v.wpi = v.loopWpi || 0;
          }
        });
      }
      // -- PEDESTRIANS ----------------------------------------------
